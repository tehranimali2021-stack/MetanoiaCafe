window.menuData = [
  {
    "category": "اسپرسوبار",
    "image": "",
    "products": [
      {
        "name": "اسپرسو ویژه",
        "subtitle": "80%/20% روبوستا",
        "price": "150،000",
        "tags": [
          "فول کافئین"
        ],
        "available": true,
        "image": "images/EspressoVIP.jpg"
      },
      {
        "name": "اسپرسو عربیکا",
        "subtitle": "%۸۰/%۲۰ عطر بالا",
        "price": "210،000",
        "tags": [
          "پریمیوم"
        ],
        "available": true,
        "image": "images/EspressoVIP.jpg"
      },
      {
        "name": "آمریکانو روبوستا",
        "subtitle": "اسپرسو به همراه آب جوش",
        "price": "200،000",
        "tags": [
          "کافئین بالا"
        ],
        "available": true,
        "image": "images/Americano.jpeg"
      },
      {
        "name": "آمریکانو عربیکا",
        "subtitle": "اسپرسو به همراه آب جوش",
        "price": "220،000",
        "tags": [
          "پریمیوم"
        ],
        "available": true,
        "image": "images/Americano.jpeg"
      },
      {
        "name": "موکا",
        "subtitle": "اسپرسو، شیر داغ و شکلات غلیظ",
        "price": "310،000",
        "tags": [
          "شکلاتی"
        ],
        "available": true,
        "image": "images/Moca.jpeg"
      },
      {
        "name": "لاته",
        "subtitle": "ترکیب کلاسیک اسپرسو و فوم شیر",
        "price": "250،000",
        "tags": [
          "محبوب"
        ],
        "available": true,
        "image": "images/Latteh.jpg"
      },
      {
        "name": "لاته نوتلا",
        "subtitle": "طعم جذاب شکلات نوتلا اصل",
        "price": "315،000",
        "tags": [
          "محبوب"
        ],
        "available": true,
        "image": "images/Latteh.jpg"
      },
      {
        "name": "لاته نوتلا توت فرنگی",
        "subtitle": "طعم جذاب شکلات نوتلا اصل به همراه سیروپ توت فرنگی",
        "price": "345،000",
        "tags": [
          "طعمی خاص"
        ],
        "available": true,
        "image": "images/Latteh.jpg"
      },
      {
        "name": "لاته دارچین عسل",
        "subtitle": "ترکیب گرم دارچین و عسل کوهی",
        "price": "325،000",
        "tags": [
          "شیرین و گرم"
        ],
        "available": true,
        "image": "images/Latteh.jpg"
      },
      {
        "name": "کاپوچینو",
        "subtitle": "اسپرسو با فوم شیر غلیظ سنتی",
        "price": "250،000",
        "tags": [
          "محبوب"
        ],
        "available": true,
        "image": "images/Moca.jpeg"
      },
      {
        "name": "کارامل ماکیاتو",
        "subtitle": "شیر، فوم، اسپرسو و سس کارامل",
        "price": "300،000",
        "tags": [
          "عاشقان کارامل"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "کورتادو",
        "price": "240،000",
        "tags": [
          "اصیل"
        ],
        "available": true,
        "image": "",
        "subtitle": "نسبت مساوی اسپرسو و شیر گرم"
      },
      {
        "name": "یونانی",
        "price": "200،000",
        "subtitle": "دم‌اوری سنتی به سبک یونانی",
        "tags": [
          "کلاسیک"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "ترک",
        "price": "160،000",
        "subtitle": "دم آوری با شن مخصوص",
        "tags": [
          "اصیل"
        ],
        "available": true,
        "image": "images/EspressoVIP.jpg"
      }
    ]
  },
  {
    "category": "نوشیدنی های گرم",
    "image": "",
    "products": [
      {
        "name": "شیرپسته داغ",
        "price": "270،000",
        "tags": [
          "طعمی خاص"
        ],
        "available": true,
        "image": "",
        "subtitle": "شیر، پودر پسته و زعفران"
      },
      {
        "name": "هات چاکلت",
        "price": "250،000",
        "tags": [
          "خوراک زمستان"
        ],
        "available": true,
        "image": "images/HotChocalate.jpeg",
        "subtitle": "شیر، پودر هات چاکلت"
      },
      {
        "name": "هات چاکلت فندقی",
        "price": "270،000",
        "tags": [
          "طعمی از دل فندق"
        ],
        "available": true,
        "image": "images/HotChocalate.jpeg",
        "subtitle": "شیر، پودر هات چاکلت فندقی، فندق"
      },
      {
        "name": "وایت چاکلت",
        "price": "250،000",
        "tags": [
          "کلاسیک"
        ],
        "available": true,
        "image": "images/Shir.jpeg",
        "subtitle": "شیر، پودر وایت چاکلت"
      },
      {
        "name": "ماسالا",
        "price": "250،000",
        "tags": [
          "ادویه‌ای"
        ],
        "available": true,
        "image": "",
        "subtitle": "شیر، پودر ماسالا"
      },
      {
        "name": "نسکافه",
        "price": "250،000",
        "image": "",
        "subtitle": "شیر، پودر نسکافه",
        "available": true,
        "tags": [
          "کلاسیک"
        ]
      },
      {
        "name": "شیرکاکائو",
        "price": "250،000",
        "image": "",
        "subtitle": "شیر، پودر کاکائو، شکر",
        "available": true,
        "tags": [
          "سنتی"
        ]
      },
      {
        "name": "شیرعسل دارچین زعفران",
        "price": "250،000",
        "tags": [
          "آرامش‌بخش"
        ],
        "available": true,
        "image": "images/Shir.jpeg",
        "subtitle": "شیر، عسل، دارچین، زعفران"
      },
      {
        "name": "چای کرک",
        "price": "250،000",
        "tags": [
          "ادویه‌ای سنتی"
        ],
        "available": true,
        "image": "",
        "subtitle": "شیر، پودر کرک"
      }
    ]
  },
  {
    "category": "نوشیدنی های سرد",
    "image": "images/ColdDrinks.jpg",
    "products": [
      {
        "name": "آیس لاته",
        "price": "290،000",
        "tags": [
          "خنک و کلاسیک"
        ],
        "available": true,
        "image": "images/Icelatte.jpeg",
        "subtitle": "ترکیب کلاسیک اسپرسو، یخ"
      },
      {
        "name": "آیس کارامل ماکیاتو",
        "price": "300،000",
        "tags": [
          "خنک و محبوب"
        ],
        "available": true,
        "image": "",
        "subtitle": "شیر، اسپرسو و سیروپ کارامل، یخ"
      },
      {
        "name": "آیس موکا",
        "price": "300،000",
        "tags": [
          "خنک و شکلاتی"
        ],
        "available": true,
        "image": "",
        "subtitle": "اسپرسو، شیرو سیروپ شکلات، یخ"
      },
      {
        "name": "آیس آمریکانو روبوستا",
        "price": "200،000",
        "tags": [
          "خنک و تلخ"
        ],
        "available": true,
        "image": "",
        "subtitle": "اسپرسو به همراه یخ "
      },
      {
        "name": "آیس آمریکانو عربیکا",
        "price": "210،000",
        "tags": [
          "خنک و طعم خاص"
        ],
        "available": true,
        "image": "",
        "subtitle": "اسپرسو به همراه یخ"
      },
      {
        "name": "موهیتو",
        "price": "300،000",
        "tags": [
          "خنک و تازه"
        ],
        "available": true,
        "image": "",
        "subtitle": "آب سودا لیمویی، لیمو، نعنا، سیروپ موهیتو، سیروپ نعنا، یخ"
      },
      {
        "name": "اسموتی انبه",
        "price": "300،000",
        "tags": [
          "خنک و تازه"
        ],
        "available": true,
        "image": "",
        "subtitle": ""
      },
      {
        "name": "آبجو",
        "price": "200،000",
        "subtitle": "آبجو، یخ، لیمو همراه با ظرف مزه (چیپس، زیتون، بادام زمینی)",
        "tags": [
          "کلاسیک"
        ],
        "available": true,
        "image": "images/Abjo.jpeg"
      },
      {
        "name": "شربت تخم شربتی و خاکشیر",
        "price": "120،000",
        "subtitle": "آبلیمو، خاکشیر، تخم شربتی، آب",
        "tags": [
          "مخصوص گرما"
        ],
        "available": true,
        "image": "images/SharbatAblimo.jpg"
      },
      {
        "name": "شربت گل محمدی",
        "price": "120،000",
        "subtitle": "آب همراه با تخم شربتی، شربت گل محمدی، زعفران",
        "tags": [
          "مخصوص گرما"
        ],
        "available": true,
        "image": "images/SharbatGolMoham.jpg"
      }
    ]
  },
  {
    "category": "شیک ها",
    "image": "",
    "products": [
      {
        "name": "شیک توت فرنگی",
        "subtitle": "شیر، بستنی، توت فرنگی، سیروپ توت فرنگی",
        "price": "360،000",
        "tags": [
          "طعمی صورتی"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "شیک بادام زمینی",
        "subtitle": "شیر، بستنی، کره بادام زمینی، بادام زمینی",
        "price": "400،000",
        "tags": [
          "کلاسیک"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "شیک موز شکلات",
        "subtitle": "شیر، بستنی، موز، نوتلا، ",
        "price": "400،000",
        "tags": [
          "طعمی شکلاتی"
        ],
        "available": true,
        "image": "images/ShakeMozChocalate.jpeg"
      },
      {
        "name": "شیک شکلات فندقی",
        "price": "400،000",
        "subtitle": "شیر، بستنی، نوتلا، فندق",
        "tags": [
          "شکلاتی"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "شیک نوتلا",
        "price": "400،000",
        "subtitle": "شیر، بستنی، نوتلا",
        "tags": [
          "شکلاتی"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "آفوگاتو",
        "price": "300،000",
        "subtitle": "اسپرسو به همراه بستنی",
        "tags": [
          "تلخ شیرین"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "آفوگاتو کاراملی",
        "price": "325،000",
        "subtitle": "اسپرسو به همراه بستنی، سیروپ کارامل",
        "tags": [
          "طعمی خاص"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "آفوگاتو شکلاتی",
        "price": "325،000",
        "subtitle": "اسپرسو به همراه بستنی، سیروپ شکلات",
        "tags": [
          "شکلاتی"
        ],
        "available": true,
        "image": ""
      },
      {
        "name": "کافه گلاسه",
        "price": "420،000",
        "subtitle": "اسپرسو به همراه بستنی، قهوه گلد و پودر نسکافه",
        "tags": [
          "طعمی خاص و کلاسیک"
        ],
        "available": true,
        "image": "images/CafeGelasse.jpeg"
      }
    ]
  },
  {
    "category": "دمنوش ها",
    "image": "",
    "products": [
      {
        "name": "دمنوش سلامتی (تقویت سیستم ایمنی بدن)",
        "price": "250،000",
        "image": "",
        "subtitle": "پونه + آویشن + عناب + چای کوهی + برگ سبز",
        "available": true,
        "tags": [
          "آرامش بخش"
        ]
      },
      {
        "name": "دمنوش آرام جان",
        "price": "250،000",
        "image": "",
        "subtitle": "گل گاوزبان، گل محمدی، بهارنارنج، لیمو",
        "available": true,
        "tags": [
          "آرامش بخش"
        ]
      },
      {
        "name": "دمنوش انرژی زا",
        "price": "280،000",
        "tags": [
          "مقوی"
        ],
        "available": true,
        "image": "",
        "subtitle": "سیب + هل + دارچین + زعفران + عسل"
      },
      {
        "name": "دمنوش لاغری",
        "price": "250،000",
        "tags": [
          "طعمی خاص و تند"
        ],
        "available": true,
        "image": "",
        "subtitle": "چای سبز + زنجبیل + دارچین + گل محمدی"
      },
      {
        "name": "دمنوش شب",
        "price": "250،000",
        "image": "",
        "subtitle": "اسطوخودوس + بابونه + به لیمو + گل محمدی",
        "available": true,
        "tags": [
          "آرامش بخش"
        ]
      },
      {
        "name": "چای ترش",
        "price": "230،000",
        "subtitle": "برگ چاش ترش",
        "tags": [
          "کلاسیک"
        ],
        "available": true,
        "image": ""
      }
    ]
  },
  {
    "category": "چای ها",
    "image": "",
    "products": [
      {
        "name": "چای سیاه",
        "price": "75،000",
        "subtitle": "چای دمی لاهیجان",
        "tags": [
          "سنتی"
        ],
        "available": true,
        "image": "images/Tea.jpeg"
      },
      {
        "name": "چای هل و دارچین",
        "price": "105،000",
        "subtitle": "چای دمی لاهیجان، دانه هل، چوب دارچین",
        "tags": [
          "معطر"
        ],
        "available": true,
        "image": "images/Tea.jpeg"
      },
      {
        "name": "چای زعفرانی",
        "price": "175،000",
        "subtitle": "چای دمی لاهیجان، پودر زعفران",
        "tags": [
          "آرامش بخش"
        ],
        "available": true,
        "image": "images/Tea.jpeg"
      },
      {
        "name": "چای زنجبیل",
        "price": "135،000",
        "subtitle": "چای دمی لاهیجان، گیاه زنجبیل",
        "tags": [
          "طعمی گرم و مقوی"
        ],
        "available": true,
        "image": "images/Tea.jpeg"
      }
    ]
  },
  {
    "category": "باقلوا ها",
    "image": "",
    "products": [
      {
        "name": "باقلوا گردویی",
        "price": "50،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/bgerdu.jpg"
      },
      {
        "name": "باقلوا پسته ایی",
        "price": "55،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/bpesteh.jpg"
      },
      {
        "name": "باقلوا نسکافه نارگیل",
        "price": "55،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Bnescafe.jpg"
      },
      {
        "name": "باقلوا بادامی",
        "price": "55،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/bgerdu.jpg"
      },
      {
        "name": "هاویج",
        "price": "300،000",
        "subtitle": "همراه با بستنی هم سرو می شود",
        "tags": [],
        "available": true,
        "image": ""
      }
    ]
  },
  {
    "category": "کیک ها",
    "image": "",
    "products": [
      {
        "name": "کیک شکلاتی",
        "price": "180،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/CakeShokolati.jpeg"
      },
      {
        "name": "تیرامیتسو",
        "price": "300،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Tiramisu.jpg"
      },
      {
        "name": "سه شیر",
        "price": "300،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Seshir.jpg"
      },
      {
        "name": "کوکی",
        "price": "120،000",
        "subtitle": "انتخابی طعم های سیب و دارچین، پروتئینی، کره بادام زمینی، دابل چاکلت، نوتلا، گردو شکلاتی",
        "tags": [],
        "available": true,
        "image": "images/Cookie.jpg"
      }
    ]
  },
  {
    "category": "صبحانه",
    "image": "",
    "products": [
      {
        "name": "تست پنیر و گردو",
        "price": "180،000",
        "subtitle": "نان تست، پنیر رژیمی، مغز گردو",
        "tags": [],
        "available": true,
        "image": ""
      },
      {
        "name": "تست رژیمی",
        "price": "280،000",
        "subtitle": "نان تست پروتئینی، کره بادام زمینی، موز، پودر دارچین",
        "tags": [],
        "available": true,
        "image": ""
      },
      {
        "name": "اوتمیل",
        "price": "350،000",
        "subtitle": "جو دوسر صبحانه ای، موز، همراه با شیر یا آب، مغز گردو، مویز، کره بادام زمینی، پودر دارچین",
        "tags": [],
        "available": true,
        "image": ""
      }
    ]
  },
  {
    "category": "نوشیدنی های متفرقه",
    "image": "",
    "products": [
      {
        "name": "آب معدنی",
        "price": "20،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": ""
      },
      {
        "name": "لایف رژیمی",
        "price": "120،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Hrejimi.jpg"
      },
      {
        "name": "لایف لیمو",
        "price": "120،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Hlemon.jpg"
      },
      {
        "name": "لایف بلوبری",
        "price": "120،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/hb.jpg"
      },
      {
        "name": "لایف میکس بری",
        "price": "120،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Hmixberry.jpg"
      },
      {
        "name": "لایف مشکی",
        "price": "120،000",
        "subtitle": "",
        "tags": [],
        "available": true,
        "image": "images/Hmeshki.jpg"
      }
    ]
  }
];
/* ===== اطلاعات کافه — فاز ۳/۴/۵ ===== */
window.cafeInfo = {
    name: "کافه متانویا",

    // آدرس متنی؛ لینک و نقشه به‌صورت خودکار از همین متن ساخته می‌شود
    address: "تهران، خیابان انقلاب، خیابان ابوریحان، نرسیده به لبافی نژاد، کافه متانویا",

    // اختیاری: اگر پر شود، جای آدرس در جست‌وجوی نقشه استفاده می‌شود
    mapQuery: "",

    // اختیاری (فاز ۴): مختصات دقیق — دقت مسیریابی را به‌شدت بالا می‌برد
    lat: null, // مثال: 32.6410
    lng: null, // مثال: 51.6680
    mapZoom: 16,

    phone: "021-66962484",
    instagram: "CAFFEE_METANOIA",
    hours: "هر روز، 07:00 تا 21:00",
    wifi: "",

      // آدرس نهایی منو — مبنای ساخت کد QR (مستقل از آدرس مرورگر)
    menuUrl: "https://metanoiacae2025-bot.github.io/MetanoiaCafe/",

    // تصاویر اسلایدر (تعداد آزاد)
    images: [
        "images/Shop1.jpg",
        "images/Shop2.jpg",
        "images/Shop3.jpg",
        "images/Shop4.jpg",
        "images/Shop5.jpg"
    ]
};
